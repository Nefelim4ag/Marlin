...
void draw_heater_status(uint16_t x, uint16_t y, const int8_t Heater) {
  ...
  tft_string.set(FPSTR(i16tostr3rj(currentTemperature)));
  if (targetTemperature > 0) {
    tft_string.add("/");
    tft_string.add(FPSTR(i16tostr3rj(targetTemperature)));
  }
...
void draw_fan_status(uint16_t x, uint16_t y, const bool blink) {
  ...
  tft_string.set(FPSTR(ui8tostr4pctrj(thermalManager.fan_speed[0])));
  tft_string.trim();
...
void draw_feedrate_status(uint16_t x, uint16_t y) {
  ...
  tft_string.set(FPSTR(i16tostr3rj(feedrate_percentage)));
  tft_string.add('%');
  tft_string.trim();
...
void draw_flowrate_status(uint16_t x, uint16_t y) {
  ...
  tft_string.set(FPSTR(i16tostr3rj(planner.flow_percentage[active_extruder])));
  tft_string.add('%');
  tft_string.trim();
  tft.add_text(tft_string.center(ITEM_WIDTH2) + 2, 68, Color, tft_string);
...
//  extern screenFunc_t _manual_move_func_ptr;
  if (ui.currentScreen != ui.manual_move.screen_ptr && !ui.external_control) {
...
void MenuItem_confirm::draw_select_screen(FSTR_P const yes, FSTR_P const no, const bool yesno, FSTR_P const pref, const char * const string/*=nullptr*/, FSTR_P const suff/*=nullptr*/) {
  ...
  #if ENABLED(TOUCH_SCREEN)
    if (no)  add_control( 328, TFT_HEIGHT - 64, CANCEL, imgCancel, true, yesno ? HALF(COLOR_CONTROL_CANCEL) : COLOR_CONTROL_CANCEL);
    if (yes) add_control(88, TFT_HEIGHT - 64, CONFIRM, imgConfirm, true, yesno ? COLOR_CONTROL_CONFIRM : HALF(COLOR_CONTROL_CONFIRM));
  #endif
}
