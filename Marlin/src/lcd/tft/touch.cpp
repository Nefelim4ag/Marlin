...
bool Touch::get_point(int16_t *x, int16_t *y) {
    ...
    #if ENABLED(TOUCH_SCREEN_CALIBRATION)
      bool is_touched = (touch_calibration.calibration.orientation == TOUCH_PORTRAIT ? TOUCH_DRIVER_CLASS.getRawPoint(y, x) : TOUCH_DRIVER_CLASS.getRawPoint(x, y));
    ...
    #else
      bool is_touched = (TOUCH_ORIENTATION == TOUCH_PORTRAIT ? TOUCH_DRIVER_CLASS.getRawPoint(y, x) : TOUCH_DRIVER_CLASS.getRawPoint(x, y));
      *x = uint16_t((uint32_t(*x) * TOUCH_CALIBRATION_X) >> 16) + TOUCH_OFFSET_X;
      *y = uint16_t((uint32_t(*y) * TOUCH_CALIBRATION_Y) >> 16) + TOUCH_OFFSET_Y;
    #endif
  #elif ENABLED(TFT_TOUCH_DEVICE_GT911)
    bool is_touched = (TOUCH_ORIENTATION == TOUCH_PORTRAIT ? TOUCH_DRIVER_CLASS.getPoint(y, x) : TOUCH_DRIVER_CLASS.getPoint(x, y));
  #endif
  #if HAS_TOUCH_SLEEP
    if (is_touched)
      wakeUp();
