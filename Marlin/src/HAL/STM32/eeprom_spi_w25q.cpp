...
void eeprom_init(void)
{
    ...
    spiflash.Init();
    //eeprom_test();
    spiflash.ReadBuff(SPI_EEPROM_OFFSET, MARLIN_EEPROM_SIZE, (uint8_t*)spi_eeprom);
}
...
{
    DEBUG("Finish EEPROM");
    spiflash.WriteBuff(SPI_EEPROM_OFFSET, MARLIN_EEPROM_SIZE, (uint8_t *)spi_eeprom);
}
...
void eeprom_test(void){
    ...
    for(uint32_t i=0; i < 50 ; ){
        ...
        spiflash.ReadBuff(SPI_EEPROM_OFFSET+i, 10, (uint8_t *)spi_eeprom); // .SPI_FLASH_BufferRead((uint8_t *)spi_eeprom,SPI_EEPROM_OFFSET+i,10);
    ...
    DEBUG("Erase flash");
    spiflash.EraseSector(SPI_EEPROM_OFFSET); // SPI_FLASH_SectorErase(SPI_EEPROM_OFFSET);
    ...
    for(uint32_t i=0; i < 50 ; ){
        ...
        spiflash.WriteBuff(SPI_EEPROM_OFFSET+i, 10, (uint8_t *)spi_eeprom); // SPI_FLASH_BufferWrite((uint8_t *)spi_eeprom,SPI_EEPROM_OFFSET+i,10);
